...
async function handleMessageSend(event = null) {
    ...
    try {
        ...
        if (res.status === 401 || res.status === 403) {
            createChatMessage("⚠️ Sesión expirada. Por favor, inicia sesión nuevamente.", true);
            setTimeout(() => {
        ...
        if (!res.ok) {
            ...
            throw new Error(errorData.error || `Error al guardar el mensaje: ${res.status}`);
        }
        ...
        if (!botResponse.ok) {
            ...
            throw new Error(errorData.error || "Error en la respuesta del chatbot");
        }
        ...
        if (data && data.message) {
            ...
            // Intentar guardar la respuesta del bot
            try {
                ...
                console.warn("No se pudo guardar la respuesta del bot:", saveError);
                // No mostrar error al usuario, ya que la respuesta se mostró correctamente
            }
        }
    } catch (error) {
        console.error("Error:", error);
        createChatMessage(`❌ ${error.message || "Hubo un problema al procesar tu mensaje. Por favor, inténtalo de nuevo."}`, true);
    }
}
...
window.cargarMensajesProyecto = cargarMensajesProyecto;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Los eventos de proyectos ahora se manejan en criterios-aceptabilidad.js
    // Solo mantener la funcionalidad del chat aquí
});
